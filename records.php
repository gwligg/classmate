...
function renderForm($modulecode = '', $type ='', $room = '', $starttime = '', $endtime = '', $dow = '', $startdate = '', $enddate = '', $error = '', $cid = '')
...
<head>
<title>
<?php if ($cid != '') { echo "Edit Class"; } else { echo "New Class"; } ?>
</title>
...
<h2 class="form-signin-heading"><?php if ($cid != '') { echo "Edit Class"; } else { echo "New Class"; } ?></h2><hr />
<?php if ($error != '') {
...
$dbcon = new mysqli($host, $username, $pass, $database);
  ...
  $stmt = $dbcon->prepare("SELECT ModuleCode,Type,Room,StartTime,EndTime,DayOfWeek,StartDate,EndDate FROM class WHERE ClassID=?");
  
  $stmt->bind_param("i", $cid);
  $stmt->execute();

  $stmt->bind_result($modulecode, $type, $room, $starttime, $endtime, $dow, $startdate, $enddate);
  $stmt->fetch();
...
<p>Class ID <?php echo $cid; ?></p>
<?php }  ?>

  <div class="form-group row">
              <label class="col-sm-6 control-label" for="inputCode">Module Code</label>
              <div class="col-sm-6">
  <select class="form-control" name="inputCode" required>
  <option selected disabled>Module Code</option>
      <?php
        include('config.php');
        $dbcon = new mysqli($host, $username, $pass, $database);
      ...
      $queryModules = "SELECT ModuleCode, ModuleTitle FROM module ";
      $dbcon = mysqli_query($dbcon, $queryModules);
      while ( $row=mysqli_fetch_assoc($dbcon)) {

      echo "<option value=".$row['ModuleCode']. '';
      if($modulecode == $row['ModuleCode']) {
        echo " selected"; }
      echo ">" . $row['ModuleCode']. ' '. $row['ModuleTitle']."</option>";
      }
    ...
    ?>
            ...
            <div class="form-group row">
              <label class="col-sm-6 control-label" for="inputType"> Class Type</label>
              <div class="col-sm-6">
               <input type="text" class="form-control" name="inputType" id="inputType" value="<?php echo $type; ?>" required />
              </div>
            </div>
            <div class="form-group row">
              <label class="col-sm-6 control-label" for="inputRoom"> Class Room</label>
              <div class="col-sm-6">
               <input type="text" class="form-control" name="inputRoom" id="inputRoom" value="<?php echo $room; ?>" required />
              </div>
              </div>            
            </div>

            <div class="form-group row">
              <label class="col-sm-6 control-label" for="inputStartTime"> Class Start Time</label>
              <div class="col-sm-6">
               <input type="" class="form-control" name="inputStartTime" id="inputStartTime" value="<?php echo $starttime; ?>" required />
              </div>
              </div>
            <div class="form-group row">
              <label class="col-sm-6 control-label" for="inputEndTime"> Class End Time</label>
              <div class="col-sm-6">
               <input type="" class="form-control" name="inputEndTime" id="inputEndTime" value="<?php echo $endtime; ?>" required />
              </div>
              </div>
            <div class="form-group row">
              <label class="col-sm-6 control-label" for="inputDow"> Day of Week</label>
                <div class="col-sm-6">
                  <select class="form-control" name="inputDow">
                    <option value="1"<?php if ($dow == 1) echo "selected"; ?>>Monday</option>
                    <option value="2"<?php if ($dow == 2) echo "selected"; ?>>Tuesday</option>
                    <option value="3"<?php if ($dow == 3) echo "selected"; ?>>Wednesday</option>
                    <option value="4"<?php if ($dow == 4) echo "selected"; ?>>Thursday</option>
                    <option value="5"<?php if ($dow == 5) echo "selected"; ?>>Friday</option>
                  </select>
                </div>
            </div>                         
            <div class="form-group row">
...
$("#inputEndTime").change(function () {
    var starttime=$('#inputStartTime').val();
    var endtime=$('#inputEndTime').val();
        if(starttime >= endtime){
          alert("End time must be later than class start time!");
...
$type = mysqli_real_escape_string($dbcon, $_POST['inputType']);
$room = mysqli_real_escape_string($dbcon,$_POST['inputRoom']);
$starttime = mysqli_real_escape_string($dbcon, $_POST['inputStartTime']);
$endtime =mysqli_real_escape_string($dbcon, $_POST['inputEndTime']);
$dow = mysqli_real_escape_string($dbcon, $_POST['inputDow']);
...
if ($modulecode == '' || $type == '' || $room == '' || $starttime == '' || $endtime == '' || $dow == '' || $startdate == '' || $enddate == '')
{
// if any empty, show an error message
$error = 'ERROR: Please fill in all required fields!';
renderForm($modulecode, $type, $room, $starttime, $endtime, $dow, $startdate, $enddate, $error, $cid);
}
...
$stmt->bind_param("sssssissi", $modulecode, $type, $room, $starttime, $endtime, $dow, $startdate, $enddate, $cid);
$stmt->execute();
$stmt->close();
}
...
if($stmt = $dbcon->prepare("SELECT ModuleCode,Type,Room,StartTime,EndTime,DayOfWeek,StartDate,EndDate FROM class WHERE ClassID=?"))
{
$stmt->bind_param("i", $cid);
$stmt->execute();
...
// show the form
renderForm($modulecode, $type, $room, $starttime, $endtime, $dow, $startdate, $enddate);
